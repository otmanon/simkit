...
_TET_RULES = {
    ...
    3: (  # exact to degree 3: 5-point rule (one negative weight)
        ...
        np.array([-4 / 5, 9 / 20, 9 / 20, 9 / 20, 9 / 20]),
    ),
...
def gauss_legendre_quadrature(V: np.ndarray, T: np.ndarray, order: int):
    ...
    bary = np.repeat(ref_points[None, :, :], t, axis=0)  # broadcast the same rule to every element
    # print(ref_points.shape)
    weights = ref_weights[None, :] * vol[:, None]  # scale the reference weights by the element volume
