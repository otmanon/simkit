...
from ...energies.elastic_energy import elastic_energy_z,  ElasticEnergyZPrecomp
from ...energies.elastic_gradient import elastic_gradient_dz
from ...energies.elastic_hessian import elastic_hessian_d2z

from ...energies.quadratic_energy import quadratic_energy
from ...energies.quadratic_gradient import quadratic_gradient
from ...energies.quadratic_hessian import quadratic_hessian

from ...energies.kinetic_energy import kinetic_energy_z,  KineticEnergyZPrecomp
from ...energies.kinetic_gradient import kinetic_gradient_z
from ...energies.kinetic_hessian import kinetic_hessian_z
...
class ElasticFEMSim():
    ...
    def __init__(self, X : np.ndarray, T : np.ndarray, B : np.ndarray | sp.sparse.csc_matrix =None, cI : np.ndarray=None,  cW : np.ndarray=None, params : ElasticFEMSimParams = ElasticFEMSimParams(), x0=None):
        """
        ...
        [self.kin_pre, self.el_pre, self.mu, self.lam, self.vol, self.mu0, self.lam0, self.vol0, self.J, self.Mv] =  \
            self.initial_precomp(X, T, B, x0, cI, cW, params.rho, params.ym, params.pr, dim)
        ...
        if params.b0 is None:
            ...
            self.b0 = self.params.b0.reshape(-1, 1)
        self.Bb0 = B.T @ self.b0


        # should also build the solver parameters
        if isinstance(params.solver_p, NewtonSolverParams):
            self.solver = NewtonSolver(self.energy, self.gradient, self.hessian, params.solver_p)
        else:
            # print error message and terminate programme
            assert(False, "Error: solver_p of type " + str(type(params.solver_p)) +
    ...
    def energy(self, z : np.ndarray):
        ...
        v = elastic_energy_z(z,  self.mu, self.lam, self.vol, self.params.material,  self.el_pre)
