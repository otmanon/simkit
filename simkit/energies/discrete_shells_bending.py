...
# Density per edge: ``ym_bending * w * (theta - theta0)^2``. The full Hessian
# includes the indefinite second-order angle term, so the ``_x`` tier exposes a
# ``psd`` flag (default ``True``).
# """
...
# def discrete_shells_bending_gradient_x(X: np.ndarray, D: np.ndarray, theta0: np.ndarray, ym_bending: np.ndarray, he: np.ndarray, le: np.ndarray) -> np.ndarray:
...
#     M = dihedral_wedge_map(D, X.shape[0])
#     Me = sp.sparse.kron(M, sp.sparse.identity(3))
#     return Me.T @ (dtheta_dx * de_dtheta).reshape(-1, 1)


# def discrete_shells_bending_hessian_x(X: np.ndarray, D: np.ndarray, theta0: np.ndarray, ym_bending: np.ndarray, he: np.ndarray, le: np.ndarray, psd: bool = True) -> sp.sparse.spmatrix:
...
#         Per-edge edge vectors.
#     psd : bool, optional
#         If ``True`` (default), project each per-edge block to PSD before
#         assembly. The full Hessian includes the indefinite second-order angle
#         term, so projection is recommended.
