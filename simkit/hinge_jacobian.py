...
def hinge_jacobian_compact(X, H):
    ...
    # Complex denominator for cross terms
    ...
    J = np.array([(Ay - By)/dist_AB_sq, 
              (-Ax + Bx)/dist_AB_sq,
              (-(Ay - Cy)*dot_prod_AB_BC - cross_prod_AB_BC*(Ax - 2*Bx + Cx))/complex_denom, 
              ((Ax - Cx)*dot_prod_AB_BC - cross_prod_AB_BC*(Ay - 2*By + Cy))/complex_denom, 
              (By - Cy)/dist_BC_sq, 
              (-Bx + Cx)/dist_BC_sq]).T
