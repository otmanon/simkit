...
def render_spring_ik_animation(Xs, Fs,  X, F, E,path, P_source=None, P_target=None,
                     ...
                     uv_type="per_vertex", save_blend_file=False, fps=30, shade_smooth=False, alpha=0.2, transmission=0.5,
    ...
    for frame in range(num_frames):
        ...
        U = X[:, :, frame]
        Us = Xs[:, :, frame]
        mesh = bt.readNumpyMesh(Us, Fs, location, rotation, scale)

        
        mesh_hidden = bt.readNumpyMesh(U, F, location, rotation, scale)
        mesh_hidden.hide_set(True)  
        mesh_hidden.hide_render = True
        ...
        if edge_scalars is not None and edge_colormap_path is not None:
            edge_objects = drawEdgeSubset_colored(mesh_hidden, E,edge_radius, edge_scalars[:, frame], edge_colormap_path)
        else:
            bt.drawEdgeSubset(mesh_hidden, E,edge_radius, edge_color)
