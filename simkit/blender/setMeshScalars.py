...
def setMeshScalars(mesh_obj, C, color_map_name='default', type=None):
    ...
    if type is None:
        if nC == nV:
            ...
            raise ValueError(
                'Error in "setMeshScalars": input scalar format must be eithe (|V|,) array of vertex scalars or (|F|,) array of face scalars')

    # check input array size
    if type is 'vertex':  # if vertex colors
        if C.shape[0] != nV:
            raise ValueError(
                'Error in "setMeshScalars": vertex scalar must have the same length as the number of vertices')
