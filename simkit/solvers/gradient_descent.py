...
def gradient_descent(x0, energy_func, gradient_func,
                            tolerance=1e-6, max_iter=1, do_line_search=True,
                            step_size=1.0, return_info=False):
    """
    Gradient descent for minimizing a scalar energy.

    At each iteration we step along the negative gradient, using either a
    backtracking line search or a fixed ``step_size``, until the step is smaller
    ...
    do_line_search : bool
        If True, run a backtracking line search to pick the step size, otherwise
        use ``step_size``.
    step_size : float
        Fixed step size used when ``do_line_search`` is False.
    return_info : bool
