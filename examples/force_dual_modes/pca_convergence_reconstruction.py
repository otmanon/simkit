...
reconstruction_errors = np.zeros((num_trials_list.shape[0], 1))
...
if not read_cache:
    for i, num_trials in enumerate(num_trials_list):
        ...
        # find argmin A || u_fdm - u_pca A ||
        A = np.linalg.solve(u_pca.T @ u_pca, u_pca.T @ u_fdm)
        reconstruction_error = np.linalg.norm(u_fdm - u_pca @ A)
        reconstruction_errors[i] = reconstruction_error

        np.save(result_dir + "_reconstruction_error.npy", reconstruction_errors)        
        print("num_trials :", num_trials)
        print("reconstruction error :", reconstruction_error)
    # print("e_val_error :", np.abs(e_val_pca[:k] - e_val).mean())
else:
    reconstruction_error = np.load(result_dir + "_reconstruction_error.npy")

# write matplotlib code to plot the cosine similarity
import matplotlib.pyplot as plt
plt.plot(num_trials_list, reconstruction_error)
plt.xlabel("#Samples")
plt.ylabel("Top 10 Principal Component Reconstruction Error")
plt.semilogx()
# make sure there are 4 x ticks
plt.xticks([ 10, 100, 1000, 10000])
plt.savefig(result_dir + "_reconstruction_error.png")
plt.savefig(result_dir + "_reconstruction_error.svg")
plt.show()
