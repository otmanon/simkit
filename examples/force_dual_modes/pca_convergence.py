...
us_pca = np.zeros((X.shape[0]*dim, num_trials_list.shape[0]))
ss_pca = np.zeros((1, num_trials_list.shape[0]))
chol_H = sp.sparse.linalg.factorized(H )

cosine_similarities = np.zeros((num_trials_list.shape[0], 1))
times = np.zeros((num_trials_list.shape[0], 1))

def compute_pca(k, chol_H, sigma_F_sqrt_e,num_trials):
...
if not read_cache:
    for i, num_trials in enumerate(num_trials_list):
        u_pca = compute_pca(k, chol_H, Sigma_F_sqrt_e, num_trials)
    
        cosine_similarity = np.diag(u_fdm.T @ u_pca)
        cosine_similarities[i] = np.abs(cosine_similarity).mean()
        
        np.save(result_dir + "cosine_similarities.npy", cosine_similarities)        
        print("num_trials :", num_trials)
        print("cosine similarity :", np.abs(cosine_similarity).mean())
    # print("e_val_error :", np.abs(e_val_pca[:k] - e_val).mean())
else:
    cosine_similarities = np.load(result_dir + "cosine_similarities.npy")

# write matplotlib code to plot the cosine similarity
import matplotlib.pyplot as plt
plt.plot(num_trials_list, cosine_similarities)
plt.xlabel("#Samples")
plt.ylabel("Top 10 Principal Component Alignment")
plt.semilogx()
# make sure there are 4 x ticks
plt.xticks([ 10, 100, 1000, 10000])
plt.savefig(result_dir + "cosine_similarity.png")
plt.savefig(result_dir + "cosine_similarity.svg")
plt.show()
