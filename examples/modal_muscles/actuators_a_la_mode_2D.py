"""Modal Muscles: 2D actuator-mode locomotion demo.

Builds a flat-function modal-muscle simulation (see ``modal_muscle_flat.py``,
the replacement for the deleted ``simkit.sims.elastic.ModalMuscleSim`` class)
and optimises a tiny set of sinusoidal actuation parameters with CMA-ES to make
a 2D creature walk forward, in the spirit of *Modal Muscles* / *Actuators a la
Mode*.

Run from the repository root::

    python examples/modal_muscles/actuators_a_la_mode_2D.py

Cached subspaces and CMA-ES histories are written to
``examples/modal_muscles/results/2d/<mesh_name>/`` (gitignored).
"""

import os
import sys

import igl
import matplotlib.pyplot as plt
import numpy as np
import scipy as sp

import simkit as sk

# Local imports next to this script; make them work from any cwd.
_SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
if _SCRIPT_DIR not in sys.path:
    sys.path.insert(0, _SCRIPT_DIR)

from animation_viewers import animation_viewer_2D  # noqa: E402
from modal_muscle_flat import build_modal_muscle, rest_state, step  # noqa: E402


def objective(Zs, B, X, T, forward_dir=np.array([1, 0]), SB=None, GAJB=None):
    z0 = Zs[:, 0]
    z1 = Zs[:, -1]
    z_diff = z1 - z0  # distance traveled throughout the simulation
    d_com = sk.subspace_com(z_diff, B, X, T, SB=SB)

    v_hat = forward_dir
    J_disp = - d_com @ v_hat

    R = sk.subspace_rotation(Zs, B, X, T, GAJB=GAJB)
    forward_dirs = (R @ forward_dir).T
    J_alignment = np.min(forward_dirs.T @ forward_dir[:, None])
    J_alignment = max(J_alignment, 0)

    J = (J_disp) * J_alignment
    return J


def simulate(s, B, p, num_timesteps=1000):
    """Simulate the modal muscle system for a set of sinusoidal parameters.

    Parameters
    ----------
    s : dict
        The modal-muscle system built by :func:`build_modal_muscle`.
    B : (dim*n, r) np.ndarray
        The skinning-eigenmode subspace.
    p : (m, 3) np.ndarray
        Sinusoidal actuation parameters: amplitude, period, phase per muscle.
    num_timesteps : int

    Returns
    -------
    Zs : (r, num_timesteps) np.ndarray
        Reduced-state history.
    """
    Zs = np.zeros((B.shape[1], num_timesteps))
    z, z_dot, a = rest_state(s, X)
    for i in range(num_timesteps):
        Zs[:, i] = z.flatten()
        t = i
        y = p[:, 0] * (np.sin(2 * np.pi * (t + p[:, 2]) / p[:, 1]))
        a[:-1] = y.sum(axis=0)
        z_next = step(s, z, z_dot, a)
        z_dot = (z_next - z) / s['h']
        z = z_next.copy()
    return Zs


class SolutionTransform():
    def __init__(self, P):
        P_mean = P.copy()
        P_mean[:, 0] *= 0
        self.p_mean = P_mean.flatten()

        P_max = P.copy()
        P_max[:, 0] = P[:, 0]
        P_max[:, 1] = P[:, 1]
        P_max[:, 2] = np.pi / 2
        self.p_max = P_max.flatten()

        P_min = P.copy()
        P_min[:, 0] = 0
        P_min[:, 1] = 0.3 * P[:, 1]
        P_min[:, 2] = 0
        self.p_min = P_min.flatten()

        P_scale = (P_max - P_min)
        self.p_scale = P_scale.flatten()

    def transform(self, dp):
        p = self.p_mean + dp * self.p_scale
        p = np.clip(p, self.p_min, self.p_max)
        return p


dir = _SCRIPT_DIR
name = "horse"
m = 6
nc = 30
k = 5
fps = 120

modeset = [3, 4]
max_s = 2
period = 50
phase = 0
dim_a = len(modeset)
num_timesteps = 500
vis_data = False
read_cache = False
data_dir = os.path.join(dir, "..", "..", "data", "2d", name)
result_dir = os.path.join(dir, "results", "2d", name)
os.makedirs(result_dir, exist_ok=True)

[X, _, _, T, _, _] = igl.readOBJ(os.path.join(data_dir, name + ".obj"))
X = X[:, :2]
X = sk.normalize_and_center(X)
X[:, 1] = X[:, 1] - np.min(X[:, 1]) + 1e-6

dim = X.shape[1]


def compute_subspace():
    [W, _E, B] = sk.skinning_eigenmodes(X, T, m)
    B = sk.orthonormalize(B, M=sp.sparse.kron(sk.massmatrix(X, T), sp.sparse.identity(dim)))
    [_E, D] = sk.linear_modal_analysis(X, T, max(modeset) + 1)

    [_cI, _cW, l] = sk.spectral_cubature(X, T, W, k, return_labels=True)
    fI = np.unique(igl.boundary_facets(T)[0])
    cfI = sk.farthest_point_sampling(X[fI, :], nc)
    cI = fI[cfI]
    return W, B, D, l, cI


[W, B, D, l, cI] = sk.filesystem.compute_with_cache_check(
    compute_subspace, os.path.join(result_dir, "subspace.npz"), read_cache=read_cache)
d = np.zeros(T.shape[0])
g = B.T @ sk.gravity_force(X, T, a=-9.8, rho=1e3).reshape(-1, 1)
D_modeset = D[:, modeset]
D_modeset_one = np.hstack([D_modeset, X.reshape(-1, 1)])
limit_a = sk.limit_actuation_dirichlet_energy(X, T, D, max_s=max_s)

if vis_data:
    LA = np.diag(limit_a)
    sk.polyscope.view_clusters(X, T, l, path=os.path.join(result_dir, 'clusters.png'))
    sk.polyscope.view_sample_points(X, T, X[cI], path=os.path.join(result_dir, 'contact_points.png'))
    sk.polyscope.view_displacement_modes(X, T, D @ LA, a=1, period=fps, fps=fps, path=os.path.join(result_dir, 'modes.mp4'))
    sk.polyscope.view_displacement_modes(X, T, D_modeset @ np.diag(limit_a[modeset]), a=1, period=fps, fps=fps, path=os.path.join(result_dir, 'actuated_modes.mp4'))


ground_height = X.min(axis=0)[1]
s = build_modal_muscle(
    X, T, B, D_modeset_one, l, d,
    mu=1e6, gamma=1e6, rho=1e3, h=1e-2, b0=-g, alpha=1.0,
    contact=True, cI=cI, plane_pos=np.array([[0, ground_height]]).T,
    max_iter=10)
[z, z_dot, a] = rest_state(s, X)

P = np.vstack([limit_a[modeset], period * np.ones(dim_a), phase * np.ones(dim_a)]).T
p_mean = P.flatten()
tr = SolutionTransform(P)
dp0 = np.zeros(P.shape[0] * P.shape[1])
d_com, SB = sk.subspace_com(np.zeros(B.shape[1]), B, X, T, return_SB=True)
R, GAJB = sk.subspace_rotation(np.zeros(B.shape[1]), B, X, T, return_GAJB=True)


def objective_func(dp):
    p = tr.transform(dp)
    Zs = simulate(s, B, p.reshape(-1, 3), num_timesteps=num_timesteps)
    r = objective(Zs, B, X, T, forward_dir=np.array([-1, 0]), SB=SB, GAJB=GAJB)
    return r


if __name__ == "__main__":

    def compute_result():
        dp_opt, result, running_history = sk.solvers.cmaes(
            dp0, objective_func, maxiter=200, popsize=16, seed=0, num_processes=8,
            return_result=True, return_history=True)
        p = tr.transform(dp_opt)
        return p, result, running_history

    p, result, running_history = sk.filesystem.compute_with_cache_check(
        compute_result, os.path.join(result_dir, "result.npz"), read_cache=read_cache)

    fbest_history = np.array([r['fbest'] for r in running_history])
    xbest_history = np.array([r['xbest'] for r in running_history])

    plt.plot(fbest_history)
    plt.title("Best Objective")
    plt.xlabel("Iteration")
    plt.ylabel("Objective")
    plt.savefig(os.path.join(result_dir, "fbest_history.png"))
    plt.show()

    Zs = simulate(s, B, p.reshape(-1, 3), num_timesteps=num_timesteps)
    r = objective(Zs, B, X, T, SB=SB, GAJB=GAJB)
    print("final reward: ", r)
    animation_viewer_2D(Zs, B, X, T, cI, ground_height)
