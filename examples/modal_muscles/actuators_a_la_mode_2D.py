...
[W, B, D, l, cI] = sk.filesystem.compute_with_cache_check(compute_subspace, os.path.join(result_dir, "subspace.npz"), 
...
if __name__ == "__main__":
    ...
    # dp_opt = np.random.randn(P.shape[0]*P.shape[1])
    # p_opt = tr.transform(dp_opt)
    # Zs = simulate(sim, p_opt.reshape(-1, 3), num_timesteps=num_timesteps)
    # r_opt = objective(Zs, B, X, T, SB=SB, GAJB=GAJB)
    # print("r_opt: ", r_opt)
    ...
    p, result, running_history = sk.filesystem.compute_with_cache_check(compute_result, os.path.join(result_dir, "result.npz"), 
                                                             read_cache=read_cache)
